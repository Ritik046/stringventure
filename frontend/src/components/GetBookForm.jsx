...
const GetBookForm = ({ defaultDetail }) => {
  ...
  return (
    <div className="ml-64 md:ml-5 sm:ml-0 sm:mt-7 sm:w-[100%]">
      {request?.bool && (
        <div className="absolute left-[42%] shadow-lg rounded-lg ease-in duration-150 text-black mt-5 bg-yellow-400 py-2 px-3 font-semibold text-lg">
          {request?.condt === "Sent" ? (
      ...
      <p className="text-5xl md:text-4xl sm:text-2xl font-bold text-blue-700 sm:text-center">
        Request Form
      </p>
      <form
        onSubmit={handleForm}
      ...
      >
        <input
          className="bg-gray-200 px-4 py-3 w-[400px] sm:w-[90%] mb-3 text-gray-800 rounded-2xl outline-none border border-gray-400"
          type="text"
        ...
        <div className="flex justify-between items-center w-[400px] sm:w-[90%] mb-3">
          <div className="flex flex-col justify-center items-start">
            <label htmlFor="from" className="text-gray-800 font-semibold">
              From *
            </label>
            <input
              onChange={handleDateChange}
              id="from"
              className="bg-gray-200 px-4 py-3 sm:text-sm mt-1 w-[180px] sm:w-[160px] mb-3 text-gray-800 rounded-2xl outline-none border border-gray-400"
              type="date"
              required
              min={min1}
            ></input>
          ...
          <div className="flex flex-col">
            <label className="text-gray-800 font-semibold">To *</label>
            <input
              onChange={(e) => setEndDate(e.target.value)}
              className="bg-gray-200 sm:text-sm px-4 py-3 mt-1 w-[180px] sm:w-[160px] mb-3 text-gray-800 rounded-2xl outline-none border border-gray-400"
              type="date"
              required
              min={min2}
            ></input>
        ...
        {/* <div className="relative">
          <p className="absolute font-semibold text-gray-800 top-3 left-5">₹</p>
          <input
            className="bg-gray-200 px-10 py-3 w-[400px] sm:w-[90%] mb-3 text-gray-800 rounded-2xl outline-none border border-gray-400"
            type="number"
            placeholder="Amount"
        ...
        <button
          type="submit"
          className="w-[400px] sm:w-[90%] bg-green-600 p-3 mt-3 text-white font-semibold rounded-xl text-lg hover:bg-green-500"
        >
          Request the Book
        </button>
      </form>
